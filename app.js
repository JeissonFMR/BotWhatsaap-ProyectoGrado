...
function start(client) {
  ...
  client.onMessage(async (message) => {
    ...
    if (message.type == "chat" || message.isForwarded) {
      ...
      let validacionCaracteres = expresionRegular.test(message.body);
      if (validacionCaracteres) {
        msjwp = message.body.toUpperCase();
      } else {
        client.sendText(message.from, '¿Comó? 🤨🤨🤨')
      }
    ...
    if (msjwp != '') {
      ...
      try {
        ...
        var data_insultos = data.insultos;
      ...
      const webScrapingRuah = async (msjruah, idData) => {
        ...
        try {
          ...
          if (msjDataAPI.includes('enviado') && msjDataAPI.includes('contraseña')) {
            ...
            client
              .reply(message.from, msjDataAPI, message.id)
              .then((result) => {
                chat.lastReceivedKey._serialized
                console.log('Result: ', result); //return object success
              }).catch((erro) => {
          ...
          client
            .sendText(message.from, errorCaragRuah)
    ...
    ¿En que te puedo colaborar ? 🤔`)
      ...
      if (msjwp === "ADEL_MUESTRA_ESTADISTICAS" && message.isGroupMsg === false) {
        estadisticas();
      } else if (msjwp === "AYUDARENCUESTA" && message.isGroupMsg === false) {
        ayudaEncuesta()
      } else if (msjwp === "OPCIONES" && message.isGroupMsg === false) {
      ...
      } else if (msjwp === "ACERCADE" && message.isGroupMsg === false) {
        if (estadoDb !== 'encuesta') {
          if (estadoDb !== 'espera') {
            acercaDe()
          }
